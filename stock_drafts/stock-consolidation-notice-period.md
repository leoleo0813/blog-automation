...
serp_check: |
  ...
  WebSearch "주식 병합 뜻" + "액면병합이란" 상위 결과 종합:
  ko.wikipedia.org(위키백과, 백과) / kbthink.com(KB금융, 대형 금융사 공식, ×2) /
  help-me.kr(헬프미, 변호사 법률 콘텐츠, ×2) / zuzu.network(스타트업 핀테크 서비스
  콘텐츠, ×2, 12·29편에서도 확인된 유형) / dic.hankyung.com(한국경제 경제용어사전,
  언론사) / namu.wiki(나무위키, 백과) / klca.or.kr(한국상장회사협의회 PDF, 준정부
  협회) / brunch.co.kr(개인 콘텐츠 플랫폼 - "초보 주식투자자 첫걸음" 시리즈) /
  namulaw.co.kr(개인 법률사무소 게시판) / blog.k-startbiz.org(법인설립대행 소규모
  서비스 콘텐츠) / richzebra.com(개인 투자 블로그)
  1) 진입 여지 — 있음. brunch.co.kr·richzebra.com·namulaw.co.kr·blog.k-startbiz.org·
     zuzu.network(×2) 등 개인/소규모 콘텐츠가 5곳 이상 상위·준상위에 진입. SERP
     안 잠김.
  2) 검색 의도 — 정보 탐색형("뜻" 정의 검색). 조회·계산기 실행이 지배적 의도가
     아님.
  3) 답 완결 여부 — 아니다. 상위 결과(위키백과·KB·헬프미·한경사전)는 병합의 정의와
     "동전주 이미지 탈피"라는 실시 목적까지만 다루고, (a) 상법상 최소 공고기간이
     며칠인지, (b) 병합비율에 따른 주식수·기준주가 계산 예시, (c) 실제 매매정지·
     재개 일정을 회사 공시에서 직접 확인하는 절차는 다루지 않음. 정보이득 여지
     뚜렷함.
  → 3개 탈락 조건 모두 미해당, 게이트2 통과.
unique_asset: |
  [완성 2026-09-14]
  (a) 액면분할과 액면병합이 반대 개념이라는 점, 그리고 자본금이 줄어드는 무상감자와는
      달리 병합은 자본금이 그대로라는 차이를 표·문장으로 명확히 정리해, 정의만
      나열하는 상위 검색 결과보다 한 단계 더 들어간다.
  (b) 계산 예시 — 5:1 병합을 예로, 보유 주식 100주가 20주로 줄고 기준주가는 5배로
      재산정돼 이론상 평가금액 총액은 병합 전후 동일하다는 점을 숫자로 보여준다.
      다만 병합 자체가 기업가치를 바꾸는 건 아니라는 현실적 한계도 함께 짚는다.
  (c) 핵심 정보이득 — 상법 제440조·제441조에 따라 회사는 주식을 병합할 때 "1개월
      이상의 기간을 정하여" 그 뜻을 공고하고 주주·질권자에게 통지해야 하며, 그
      기간이 만료된 때 병합의 효력이 발생한다는 법정 최소 기간을 정리한다. 상위
      검색 결과 어디에도 이 법정 공고기간의 구체적 일수는 다루지 않는다.
  (d) 다만 거래소 매매거래정지의 정확한 시작·종료일(며칠 정지되는지)은 회사·종목마다
      달라 이 글에서 특정 일수를 단정하지 않고, 대신 DART·KIND 공시에서 신주배정
      기준일과 변경(신주)상장 예정일을 직접 찾아 스스로 확인하는 절차를 안내한다.
      "며칠 정지되나요"라는 독자의 실제 궁금증에 안전하게(수치를 지어내지 않고)
      답하는 방식 자체가 정보이득이다.
primary_source: |
  1차 시도: 국가법령정보센터(law.go.kr)의 상법 제440조 조문정보 페이지에 WebFetch
  1회 시도 → EGRESS_BLOCKED(2026-09-14, 이 세션의 기존 전면 차단 패턴과 일치).
  대조군으로 헬프미(help-me.kr) 블로그에도 WebFetch 1회 추가 시도 → 동일하게
  EGRESS_BLOCKED, 세션 전면 차단으로 확인.
  RULES.md 「1차 출처가 막혔을 때: 2차 출처 교차검증 vs 사람 캡처 요청」(2026-09-12)
  기준 적용 — 상법 제440조 "1개월 이상의 기간" 문구가 아래 4곳에서 충돌 없이
  일치했다.
  ① law.go.kr(국가법령정보센터) — WebSearch 스니펫에 조문 원문 "1월 이상의 기간을
     정하여 그 뜻과 그 기간내에 주권을 회사에 제출할 것을 공고" 그대로 노출.
     소관 부처(법무부) 원문이자 이 조문의 원 출처.
  ② casenote.kr — 법령·판례 데이터베이스, 상법 제440조·제441조·제442조 전문을
     동일 문구로 인용.
  ③ lawmeca.com(법률메카, 법률QA 플랫폼) — 상법 440·441조의 적용 범위를 설명하며
     동일한 "1개월 이상" 요건을 확인.
  ④ help-me.kr(헬프미, 변호사 작성 법률 콘텐츠) — "액면분할과 액면병합: 절차,
     필요서류 및 주의사항" 글에서 같은 절차를 독립적으로 서술(WebSearch 스니펫
     기준, 본문 WebFetch는 차단됨).
  법무부 소관 성문법 조문(상법 440~442조)으로 개정 빈도가 낮고 최근 변경 이력도
  검색되지 않아, 세율·공제한도류의 민감 수치와 달리 안정적으로 서술 가능하다고
...
gate_pass_note: |
  ...
  게이트1: 네이버 키워드도구 실측 1,760회(backlog.verified에 이미 게이트1 통과
  상태로 대기 중이던 항목 중 검색량 최고 - "다음 실행 후보 1순위"로 명시돼 있던
  "주식 병합 뜻"을 이번 편으로 승격).
  게이트2: v3 기준 통과(serp_check 참조 — 개인/소규모 콘텐츠 5곳 이상 진입, 정의형
  검색의도, 답 미완결).
  게이트3: 액면분할·무상감자와의 구조 비교 + 병합비율 계산 예시 + 상법상 법정
  공고기간(1개월 이상) + DART/KIND 공시 확인 절차로 상위 검색 결과에 없는
  정보이득 확보.
  게이트4: law.go.kr·help-me.kr 각 1회 WebFetch 시도 모두 EGRESS_BLOCKED 확인 후,
  법무부 소관 원문 스니펫 + 법률DB(casenote.kr) + 법률QA(lawmeca.com) + 로펌
  콘텐츠(help-me.kr) 4곳 교차검증으로 진행. 실제 매매정지 일수는 단정하지 않고
  공시 확인 절차로 안내. 한계는 primary_source·self_check에 투명 공개.
self_check: |
  [2026-09-14 최종 판정]
  게이트1 충족 — backlog.verified의 "주식 병합 뜻"(1,760회, 2026-09-14 실측)이
  hold_reason "다음 실행 후보 1순위"로 이미 게이트1을 통과한 상태였다. 같은 목록의
  "자본잠식 뜻"(1,400회)보다 검색량이 높아 이번 편으로 우선 승격했다. 별도 신규
  키워드 브레인스토밍·check-keywords.yml 실행은 이번 편에서는 불필요했다.
  게이트2 충족 — RULES.md 게이트2 v3 기준, 3개 탈락 조건 모두 미해당(serp_check
  참조).
  게이트3 충족 — 순수 사전적 정의(주식병합 뜻)만으로는 RULES.md의 "사전형 개념은
  정보이득 불가" 원칙에 걸리므로, 여기에 "상법상 법정 공고기간"과 "실제 매매정지
  일정을 공시로 직접 확인하는 법"이라는 실전 확인법을 결합해 통과시켰다(17편
  공매도 뜻, 26편 블록딜 뜻, 27편 숏커버링 뜻, 29편 감자 뜻과 동일한 "뜻+실전
  확인법" 패턴).
  게이트4 — law.go.kr·help-me.kr에 각 1회 WebFetch 시도해 모두 EGRESS_BLOCKED
  확인(2026-09-14, 이 세션 기존 패턴과 일치). RULES.md 2026-09-12 기준에 따라
  핵심 문구("1개월 이상의 기간")가 법무부 소관 원문 스니펫과 독립된 법률DB
  (casenote.kr)·법률QA(lawmeca.com)·로펌 콘텐츠(help-me.kr) 3곳에서 충돌 없이
  일치해 캡처 요청 없이 진행했다. 다만 이 숫자는 "구주권 제출 공고기간"의 법정
  최소값이지, 거래소의 실제 매매거래정지 일수(종목마다 다름)와는 다른 개념임을
  본문에서 명확히 구분했고, 실제 정지 일수는 어디에서도 3곳 이상 일치하는 확실한
  수치를 찾지 못해 단정하지 않았다(대신 공시 확인 절차로 안내) — 이는 "세율·
  공제한도"류처럼 이 프로젝트가 과거 오류를 잡아낸 유형의 숫자를 추측하지 않기
  위한 판단이다.
  카니벌라이제이션 점검 — 1~30편 어디에도 주식병합(액면병합)은 다루지 않는다.
  29편(감자 뜻)과는 "자본금 변동 여부"가 핵심적으로 다른 별개 제도이며, 본문에서
  이 차이를 직접 비교해 오히려 상호 참조 가치를 높였다.
  기관 링크 점검(RULES.md「기관 링크 필수」) — 본문에서 국가법령정보센터·
  전자공시시스템·한국거래소를 안내하는 자리와 하단 참고 출처 전부 target="_blank"
