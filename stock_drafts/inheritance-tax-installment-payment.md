...
unique_asset: |
  ...
  (c) 14편(가업상속공제 한도와 요건)에서 이미 확정한 경영기간별 공제한도와
      ...
      확인, 0건).
  (d) 정확한 연부연납 가산율(이자율)은 출처 간 수치가 상충해 캡처 대기 —
      아래 primary_source·capture_guide 참조.
primary_source: |
  ...
  본다.
  다만 연부연납 가산금의 구체적 이자율(가산율)은 자료군마다 다른 값이 나와
  source_conflict에 남기고 캡처 대기로 전환했다.
  근거 법령: 상속세및증여세법 제71조(연부연납), 제72조(연부연납가산금).
source_conflict: |
  연부연납 가산율(이자율) — 자료군 A는 "2023-03-20 이후 연 2.9%, 이후 어느
  3월 16일부터 연 1.2%로 인하"라고 서술(적용 연도 불명확). 자료군 B는
  "국세기본법 시행규칙 제19조의3, 2025-03-21 개정 이후 연 3.1%가 2026년
  5월 현재까지 적용"이라고 서술(법령 조문 근거 명시). 두 값이 상충한다.
  자료군 B가 더 구체적이지만, 사람이 국세상담센터 Q&A 또는 국세법령정보
  시스템에서 2026-09 시점 기준 가산율을 직접 확인하기 전까지 본문에 확정
  수치로 쓰지 않는다. 확정 전까지 gate_pass:false.
기준일: 2026-09-21 (WebSearch 교차검증 확인일 기준, 가산율 제외)
tags: 상속세연부연납, 증여세연부연납, 상속세납부, 가업상속공제, 상속세담보, 주식초보
gate_pass: false
gate_pass_note: |
  게이트1·2·3(부분)·4(부분) 충족. 신청기준(2천만원)·담보 종류와 비율
  (120%/110%)·상장주식 자동승인·기간(10년/20년/5년)·허가통지기한(6개월/
  3개월/자동허가)은 독립 출처 8곳 이상 교차검증으로 충족. 다만 연부연납
  가산율(이자율) 수치가 출처 간 상충해(2.9%→1.2% vs 3.1%) RULES.md
  「세율·공제한도처럼 과거 오류를 잡아낸 유형의 숫자」 기준에 해당하는
  것으로 판단, 이 값만 비워두고 gate_pass:false로 저장한다. 캡처 후 이자
  계산 예시를 완성하면 재판정한다.
capture_guide: |
  (1) 왜 필요한가 — 상속세·증여세 연부연납 시 매년 붙는 가산금 이자율
  (가산율)이 정확히 몇 %인지 출처마다 다르게 나옵니다(한쪽은 1%대, 다른
  쪽은 3.1%). 이 숫자가 있어야 "담보를 얼마나 더 준비해야 하는지", "이자
  부담이 실제로 얼마인지" 계산 예시를 완성할 수 있습니다.
  (2) 시도할 사이트 (우선순위)
    1순위 — 국세상담센터 Q&A
    (https://call.nts.go.kr/call/qna/selectQnaInfo.do?mi=2060&ctgId=CTG11668)
    에서 "연부연납 가산금 이자율"을 검색해 현재 적용 이자율이 나온 화면을
    캡처.
    2순위 — 국세법령정보시스템(https://taxlaw.nts.go.kr)에서 "국세기본법
    시행규칙 제19조의3"을 검색해 현재 고시된 가산율(연 1천분의 몇) 조문
    원문을 캡처.
    3순위 — 국세청 상속세 납부 안내 페이지
    (https://www.nts.go.kr/nts/cm/cntnts/cntntsView.do?mi=2331&cntntsId=7724)
    에서 연부연납 관련 설명을 캡처.
  (3) 캡처가 끝나면 — 스크린샷을 대화에 올려주세요. 정확한 가산율로 이자
  계산 예시를 채우고 gate_pass를 재판정합니다.
self_check: |
  ...
  참조).
  게이트3 부분 충족 — 담보 종류·비율 계산 예시, 상장주식 자동승인이라는
  이 시리즈 특화 정보이득, 14편과의 20년 특례 연결까지 확정. 다만 이자
  계산 자체는 가산율 확인 후 완성 예정.
  게이트4 부분 충족(교차검증) — taxlaw.nts.go.kr·call.nts.go.kr 각 1회
  시도 EGRESS_BLOCKED, 대조군 kofia.or.kr도 동일 차단으로 세션 전면 차단
  확인. 신청기준·담보·기간·허가절차는 독립 출처 8곳 이상이 충돌 없이
  일치해 교차검증 조건을 충족했으나, 가산율만 두 자료군이 상충해(2.9%→
  1.2% vs 3.1%) RULES.md 기준상 캡처가 필요한 숫자로 판단했다.
  ...
  종합 판정: 게이트4(가산율) 미충족으로 gate_pass:false. 캡처 후 재판정
  필요.
---

<p>상속세나 증여세 납부세액이 2,000만원을 넘으면, 한 번에 내지 않고 최대 10년(증여세는 5년)에 걸쳐 나눠 내는 <mark>연부연납</mark>을 신청할 수 있습니다. 신청하려면 담보 제공이 필수이고 매년 가산금(이자)이 붙습니다. 상속받은 주식만으로 담보를 대신할 수 있는지까지 정리했습니다.</p>
...
<div style="background:#eafaf1;border:2px solid #2e8b57;border-radius:10px;padding:16px 20px;margin:24px 0;">
  ...
  <ul style="margin:10px 0 0 0;padding-left:20px;line-height:1.9;">
    ...
    <li>기간은 상속세 10년(가업상속 최대 20년), 증여세 5년으로 서로 다릅니다.</li>
  </ul>
</div>
...
<div style="background:#f6f6f4;border-left:4px solid #999;padding:14px 18px;margin:20px 0;line-height:1.9;">
  <b>이자상당액(가산율)은 캡처 확인 중입니다</b>
  <p style="margin:8px 0 0 0;">이 글 조사 시점에 출처마다 다른 가산율(1%대~3%대)이 나와, 정확한 현재 이자율은 사람이 국세상담센터 원문을 직접 확인한 뒤 갱신할 예정입니다. 위 표는 원금 기준 비율만 참고해 주시기 바랍니다.</p>
</div>
...
<p>담보로 쓸 자산이 있는지부터 확인해 보시기 바랍니다. 상속받은 주식이나 국채가 있다면 별도 담보를 마련할 필요 없이 그대로 활용할 수 있어 훨씬 수월합니다. 정확한 이자 부담은 시점마다 달라지므로, 신청 직전에 <a href="https://call.nts.go.kr/call/qna/selectQnaInfo.do?mi=2060&amp;ctgId=CTG11668" target="_blank" rel="noopener">국세상담센터</a>나 관할 세무서에서 최신 이자율을 다시 확인하는 편이 안전합니다.</p>
...
<div style="border-top:1px solid #ddd;margin-top:32px;padding-top:12px;font-size:13px;color:#888;">
  ...
  <ul style="margin:6px 0 0 0;padding-left:20px;">
    ...
    <li><a href="https://call.nts.go.kr/call/qna/selectQnaInfo.do?mi=2060&amp;ctgId=CTG11668" target="_blank" rel="noopener">국세청 국세상담센터 - 연부연납 가산금 이자율 Q&A</a></li>
  </ul>
  기준일: 2026년 9월 기준(가산율 제외, 사람 캡처 확인 후 갱신 예정). 세율·한도는 개정될 수 있으니 신청 전 최신 내용을 다시 확인하시기 바랍니다.
</div>

<p style="font-size:13px;color:#777;margin-top:16px;line-height:1.8;">
이 글은 상속세·증여세 납부 방법을 안내하는 정보성 콘텐츠이며, 특정 절세 전략이나 금융상품 가입을 권하지 않습니다. 실제 신청 여부와 담보 제공 방법은 개별 사정에 따라 달라질 수 있으므로, 최종 판단과 그 결과는 신청인 본인에게 있습니다. 이자율 등 일부 수치는 확인 중이니 신청 전 반드시 세무서나 국세상담센터에서 최신 내용을 다시 확인하시기 바랍니다.
</p>
