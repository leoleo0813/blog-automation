...
unique_asset: |
  [계획 — 캡처 후 완성] 납입액이 아니라 "실제로 얼마 돌려받나"를 소득 구간별
  (16.5%/13.2%)로 계산해 보여주는 예시 + IRP 합산 한도와의 관계 정리.
  단, 계산에 쓰이는 한도·공제율·소득구간 수치 자체가 원문 캡처 전이라 아직
  확정되지 않았다 — 현재는 표·계산 뼈대만 있고 값이 비어 있다.
primary_source: |
...
tags: 연금저축세액공제, 연금저축, IRP세액공제, 연금계좌, 세액공제한도, 노후준비, 연말정산, 주식초보, 절세, 2026세금
gate_pass: false
gate_pass_note: |
...
self_check: |
  ...
  게이트3 — 미충족. 계산 예시에 쓸 한도·공제율 수치가 아직 원문 미확정이라
  표를 채우지 않고 뼈대만 두었다. 캡처 없이 gate_pass를 true로 만들지 않았다.
...
<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Article",
  "headline": "연금저축 세액공제 한도와 환급액",
  "description": "연금저축·IRP 세액공제 한도와 소득 구간별 공제율, 실제 환급액 계산 방법을 정리합니다. (수치는 원문 캡처 후 확정)",
