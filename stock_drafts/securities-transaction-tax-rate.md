...
gate1_pass: 확인필요 (큐 참고값 월 1,030 — 커밋 후 워크플로가 네이버 API로 실측 기입)
serp_check: |
...
primary_source: 미확보 — easylaw.go.kr WebFetch가 EGRESS_BLOCKED(2026-09-07). RULES.md「1차 출처 접근」대로 1회 시도 후 human-assisted/capture로 분류. WebSearch 요약에 2026-01-01 환원과 시장별 세율로 보이는 숫자가 나오지만 검색엔진 합성 문장이라 1차 출처가 아니므로 본문에 쓰지 않았다.
...
self_check: |
  ...
  게이트3 부분 충족 — "코스피는 증권거래세율만 보면 안 된다(농특세 별도 부과)"는 구조
  설명과 2026-01-01 환원 맥락은 서술로 넣었으나, 이를 뒷받침할 세율표 두 개가 비어 있다.
  RULES.md「실측표 자리를 비워두면 false」에 해당한다.
  게이트4 미충족 — easylaw.go.kr WebFetch EGRESS_BLOCKED. 1회만 시도하고 반복하지 않았다.
  검색 요약에 나온 숫자(코스피 0.05%+농특세 0.15%=0.20%, 코스닥 0.20% 등)는 검색엔진
  합성 문장이라 본문에 일절 쓰지 않았다.
...
<div style="background:#fdeaea;border-left:4px solid #d9534f;padding:14px 18px;margin:20px 0;line-height:1.8;">
  ...
  <p style="margin:8px 0 0 0;">코스피 세율을 설명하면서 <b>증권거래세만 적고 농어촌특별세를 빠뜨린 글이 많습니다.</b> 그런 표만 보면 실제로 떼이는 금액을 실제보다 적게 예상하게 됩니다. 코스피는 반드시 두 세금을 합쳐서 확인하세요.</p>
</div>
