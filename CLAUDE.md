...
[2] 검색량 확인 (자동)     네이버 검색광고 API 키워드도구 — 발행 후 워크플로가 자동 기입
...
- **게이트 4 제약 — 1차 출처 자동 수집의 한계 (2026-09-03 실측):** 기관마다 결과가 다르므로 주제를 고를 때 미리 고려한다.
  | 기관 | 자동 수집 | 확인된 증상 |
  |---|---|---|
  | 금융투자협회 (kofia.or.kr) | ✅ 보도자료는 수집됨 | 단, 비교공시(dis.kofia)는 WebSquare JS 앱이라 데이터 없음 |
  | 기획재정부 (moef.go.kr) | ❌ | GitHub Actions에서 connect timeout — 해외 IP 차단으로 추정 |
  | 정책브리핑 (korea.kr) | ❌ | 동일하게 connect timeout |
  | 국가법령정보센터 (law.go.kr) | ❌ | 본문이 iframe 안이라 렌더링해도 제목만 |
  - 콘텐츠 생성 샌드박스는 egress가 막혀 있고, GitHub Actions는 네트워크는 열려 있으나 위 기관들이 해외 IP를 거부한다. **따라서 세금·제도 주제의 세율·한도 수치는 현재 자동으로 확보할 수 없다.**
  - 이럴 때 취할 행동: ① 수치를 지어내지 않는다 ② 사람에게 원문 확인을 요청하고 `sources/`에 저장한 뒤 인용한다 ③ 그때까지는 수치가 필요 없는 각도(절차·구조·확인 방법)로 쓰거나 초안을 보류한다.
  - WebSearch 결과(뉴스·블로그)는 사실 확인의 단서로만 쓰고, 본문 출처로 인용하지 않는다.
- **게이트 1 자동 기입 경로:** 콘텐츠 생성 세션에는 네이버 시크릿이 없으므로, 초안을 커밋한 뒤 `notify-repo-only.yml` 워크플로가 `blog_automation.fill_search_volume` 을 실행해 키워드도구 API로 월간 검색량을 조회하고 초안 머리말의 `monthly_search_volume` 과 `gate1_pass` 를 채워 되커밋한다. 그 결과가 카카오톡 알림에도 함께 온다.
  - 콘텐츠 생성 단계에서는 `monthly_search_volume: 확인필요` 로 두고, **자동화가 `gate_pass` 를 스스로 `true` 로 만들지 않는다.** `gate_pass` 는 4개 게이트를 사람이 확인한 뒤 바꾸는 최종 판단값이다.
  - 임계값은 키워드로 자동 판별한다 — 세금·세율·공제·한도·ISA·연금·신고·과세가 들어가면 세부·제도 주제(월 100), 그 외는 일반 주제(월 500).
  - 시크릿(`NAVER_AD_CUSTOMER_ID` / `NAVER_AD_ACCESS_LICENSE` / `NAVER_AD_SECRET_KEY`)이 없거나 조회가 실패하면 `확인필요` 가 그대로 남는다 — 그때는 사람이 키워드도구에서 직접 확인한다.

### 7.2 발행 전 게이트 — 하나라도 실패하면 저장만 하고 발행하지 않는다
초안 최상단에 반드시 이 형식으로 기록한다.
```yaml
keyword: 증권사 수수료 비교
monthly_search_volume: 확인필요          # 게이트 1 — 워크플로가 자동으로 덮어쓴다
gate1_pass: false                        # 게이트 1 판정도 워크플로가 채운다
serp_check: 개인블로그 4 / 증권사·언론 4   # 게이트 2
unique_asset: 증권사 12곳 수수료율 실측표  # 게이트 3
primary_source: 금융투자협회 공시 (2026-09-01)  # 게이트 4
gate_pass: false                         # 사람이 4개 게이트를 확인한 뒤 바꾸는 최종 판단값
```
